    ...
    <>
      ...
      >
        ...
        <div className="flex items-center space-x-2">
          <Truck
            className={`text-white transition-all duration-200 ${isHovered ? "w-12 h-12" : "w-10 h-10"
              }`}
          />
          {isHovered && (
        ...
        <nav className="flex flex-col gap-4 mt-6">
          {sidebarItems.map(({ label, icon: Icon, href }) => {
            ...
            return (
              <Link key={label} href={href} className={linkClasses}>
                <Icon
                  className={`transition-transform duration-200 ${isHovered ? "scale-120" : "scale-100"
                    }`}
          ...
          <div className="mt-auto mb-6">
            <Link href="/login" className="flex items-center gap-2 text-lg font-medium cursor-pointer border-l-4 border-transparent hover:border-white hover:bg-[#7C7FE0] hover:rounded-full hover:scale-105 hover:shadow-md transition-colors transition-transform duration-200 px-1">
              <LogOut
                className={`transition-transform duration-200 ${isHovered ? "scale-120" : "scale-100"
                  }`}
                size={isHovered ? 34 : 28}
              />
              {isHovered && <span className="whitespace-nowrap">Log Out</span>}
            </Link>
          </div>
        </nav>
      </div>
